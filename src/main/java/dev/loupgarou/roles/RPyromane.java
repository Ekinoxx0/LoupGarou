...
public class RPyromane extends Role{
	...
	protected void onNightTurnTimeout(LGPlayer player) {
		if(first != null) {
			...
			MainLg.debug(getGame().getKey(), "Gasoil of "+event.getPlayer().getName()+" cancelled : "+event.isCancelled());
	...
	public void onInventoryClick(InventoryClickEvent e) {
		...
		if(item.getItemMeta().getDisplayName().equals(nothing.getItemMeta().getDisplayName())) {
			...
			if(lgp.getCache().<List<LGPlayer>>get(CacheType.PYROMANE_ESSENCE).size() != 0) {
				...
				MainLg.debug(getGame().getKey(), liste+" < liste des joueurs à kill par le pyro");
				for(LGPlayer scndPlayer : liste) {
					MainLg.debug(getGame().getKey(), scndPlayer.getName()+" mort: "+scndPlayer.isDead()+" & player: "+scndPlayer.getPlayer()+" / role:"+scndPlayer.getRole());
