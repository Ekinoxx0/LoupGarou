...
public class RolesCmd extends SubCommand {
		...
		if (args.length == 1 || args[1].equalsIgnoreCase("list")) {
			...
			for (String role : getMain().getRoles().keySet()) {
				if (MainLg.getInstance().getConfig().getInt("role." + role) > 0) {
					...
					cs.sendMessage(MainLg.getPrefix() + "  §e- " + index++ + " - §6" + role + " §e> "
							+ MainLg.getInstance().getConfig().getInt("role." + role));
				}
			}
			cs.sendMessage("\n" + MainLg.getPrefix()
					+ " §7Écrivez §8§o/lg role set <role_id/role_name> <nombre>§7 pour définir le nombre de joueurs qui devrons avoir ce rôle.");
			...
			if (args[1].equalsIgnoreCase("set") && args.length == 4) {
				...
				cs.sendMessage(MainLg.getPrefix()
						+ "§4Essayez §c/lg roles set <role_id/role_name> <nombre>§4 ou §c/lg roles list");
			}
		}
	}
	...
	public List<String> onTabComplete(CommandSender cs, LoupGarouCommand cmd, String[] args) {
		if (args.length == 2)
			return Arrays.asList("list", "set", "all");
		else if (args.length == 3 && args[1].equalsIgnoreCase("set"))
			return Arrays.asList(getMain().getRoles().keySet().toArray(new String[getMain().getRoles().size()]));
